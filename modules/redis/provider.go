...
func (p *provider) DB() *redis.Client {
  ...
  c, _ := p.Open(context.TODO(), p.Cfg.DB)
  return c
}

func (p *provider) Open(ctx context.Context, db int) (*redis.Client, error) {
  p.Log.Info("opening redis")
  p.lock.Lock()
  defer p.lock.Unlock()
